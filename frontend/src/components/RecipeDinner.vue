          ...
          <!-- <div class="flex flex-col items-center md:items-start w-[20%]">
            ...
            >
              <div
                ...
                :style="{ width: ((dinner.rating?.ratingValue || 0) / 5) * 100 + '%' }"
              ></div>
            </div>
          </div> -->
...
const fetchMealPlan = async () => {
  ...
  try {
    ...
    if (storedMealPlan) {
      ...
      if (data && data.meals && data.meals.dinner) {
        dinner.value = data.meals.dinner
        loading.value = false
        return
      }
    }
    ...
    if (!data || !data.meals || !data.meals.dinner) {
      throw new Error('Keine Abendessen-Daten verfügbar')
    }
