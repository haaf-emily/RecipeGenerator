  ...
  <div class="min-h-screen py-10 bg-gray-100 dark:bg-gray-900 transition-colors duration-300">
    ...
    <div class="container mx-auto px-6">
      ...
      <div v-if="isLoading" class="flex flex-col justify-center items-center h-screen">
        <div class="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-[#E89BA7]"></div>
      ...
      <div v-else>
        ...
        <!-- Error message -->
        <div v-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative my-8">
          <strong class="font-bold">Fehler!</strong>
          <span class="block sm:inline"> {{ error }}</span>
          <button @click="loadMealPlan(true)"
        ...
        <!-- Meal plan info -->
        <div v-else-if="mealPlanData" class="mb-8 p-4 bg-white dark:bg-[#2C3E50] rounded-lg shadow flex justify-center">
        ...
        <div v-if="!error" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div
            ...
            <img :src="mealPlanData && mealPlanData.meals
              ? mealPlanData.meals.breakfast?.image_urls?.[0] || ''
              : ''
              " :alt="mealPlanData && mealPlanData.meals
            ...
            <div class="p-6">
              ...
              <h3 v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.breakfast"
                class="font-bold mt-2 text-[#4A5759] dark:text-gray-200">
                {{ mealPlanData.meals.breakfast.title }}
              </h3>
              <p v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.breakfast"
          ...
          <div
            ...
            <div class="p-6">
              ...
              <h3 v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.lunch"
                class="font-bold mt-2 text-[#4A5759] dark:text-gray-200">
                {{ mealPlanData.meals.lunch.title }}
              </h3>
              <p v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.lunch"
                class="text-sm text-[#4A5759] mt-1 dark:text-gray-300">
          ...
          <div
            ...
            <div class="p-6">
              ...
              <h3 v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.dinner"
                class="font-bold mt-2 text-[#4A5759] dark:text-gray-200">
                {{ mealPlanData.meals.dinner.title }}
              </h3>
              <p v-if="mealPlanData && mealPlanData.meals && mealPlanData.meals.dinner"
                class="text-sm text-[#4A5759] mt-1 dark:text-gray-300">
        ...
        <div v-if="!error" class="flex justify-center mt-12">
          <button @click="regenerateMealPlan"
