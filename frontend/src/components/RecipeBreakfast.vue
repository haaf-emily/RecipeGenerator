        ...
        <div class="flex flex-col md:flex-row items-center justify-center gap-8 w-full">
          ...
          <!-- <div class="flex flex-col items-center md:items-start w-[20%]">
            <p class="text-[#E89BA7] font-bold mb-2 text-lg dark:text-[#FF9F7F]">
              {{ breakfast.rating?.ratingValue || 0 }}
            </p>
            <div
            ...
            >
              <div
                ...
                :style="{ width: ((breakfast.rating?.ratingValue || 0) / 5) * 100 + '%' }"
              ></div>
            </div>
          </div> -->
  ...
  try {
    ...
    if (storedMealPlan) {
      ...
      if (data && data.meals && data.meals.breakfast) {
        breakfast.value = data.meals.breakfast
        loading.value = false
        return
      }
    }
    ...
    if (!data || !data.meals || !data.meals.breakfast) {
      throw new Error('Keine Frühstücksdaten verfügbar')
    }
